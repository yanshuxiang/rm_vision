...
int Camera::init() {
    ...
    this->status = setProperty(8000.00,1280,1000,15.0);
...
int Camera::setProperty(double exposureTime, int width, int height,double GainRaw) {
    ...
    // this->status = IMV_SetDoubleFeatureValue(devHandle, "GainRaw", GainRaw);
    // if (IMV_OK != this->status)
    // {
    //     printf("Set ainRaw value failed! ErrorCode[%d]\n", this->status);
    //     return this->status;
    // }


    // 设置增益为连续自动模式 (Continuous)
    // 注意：GainAuto 是一个枚举(Enumeration)类型属性
    this->status = IMV_SetEnumFeatureValue(this->devHandle, "GainAuto", 2);

    if (IMV_OK != this->status) {
        printf("设置自动增益失败! ErrorCode[%d]\n", this->status);
    }
        return IMV_OK;
    }
